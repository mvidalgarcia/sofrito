...
export function saveRecipe(recipe: Recipe, status: RecipeStatus): void {
  ...
  const existing: (Recipe & { status: RecipeStatus; createdAt: string })[] = JSON.parse(localStorage.getItem(key) || '[]');
...
export function getRecipeById(id: string): (Recipe & { status: RecipeStatus; createdAt: string }) | null {
  const all = getAllRecipes();
...
export function deleteRecipe(id: string): void {
  const key = 'sofrito_recipes';
  const existing: (Recipe & { status: RecipeStatus })[] = JSON.parse(localStorage.getItem(key) || '[]');
